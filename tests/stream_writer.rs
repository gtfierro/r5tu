use rdf5d::{StreamingWriter, writer::WriterOptions, reader::R5tuFile, Term, Quint};

#[test]
fn streaming_writer_roundtrip_interleaved_order() {
    let mut path = std::env::temp_dir(); path.push("stream_roundtrip.r5tu");
    ...
    let s1 = Term::Iri("http://ex/s1".into()); let s2 = Term::Iri("http://ex/s2".into());
    let p1 = Term::Iri("http://ex/p1".into()); let p2 = Term::Iri("http://ex/p2".into());
    ...
    w.add(Quint{ id: "src/B".into(), s: s2.clone(), p: p1.clone(), o: o3.clone(), gname: "g".into()}).unwrap();
    w.add(Quint{ id: "src/A".into(), s: s1.clone(), p: p2.clone(), o: o2.clone(), gname: "g".into()}).unwrap();
    w.add(Quint{ id: "src/A".into(), s: s1.clone(), p: p1.clone(), o: o1.clone(), gname: "g".into()}).unwrap();
    ...
    for (s,p,o) in triples_a { set_a.insert((f.term_to_string(s).unwrap(), f.term_to_string(p).unwrap(), f.term_to_string(o).unwrap())); }
    let mut expected_a = std::collections::HashSet::new();
    expected_a.insert(("http://ex/s1".to_string(), "http://ex/p1".to_string(), "\"v1\"".to_string()));
    expected_a.insert(("http://ex/s1".to_string(), "http://ex/p2".to_string(), "\"v2\"@en".to_string()));
