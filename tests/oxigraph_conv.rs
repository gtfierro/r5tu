...
fn to_oxigraph_graph_basic() {
    ...
    let o1 = Term::Literal { lex: "v1".into(), dt: None, lang: Some("en".into()) };
    let q = Quint{ id:"src/A".into(), s:s1, p:p1, o:o1, gname:"g".into() };
    let mut path = std::env::temp_dir(); path.push("oxigraph_conv.r5tu");
    ...
    let triples: Vec<_> = f.oxigraph_triples(gr.gid).unwrap().collect::<Result<_,_>>().unwrap();
