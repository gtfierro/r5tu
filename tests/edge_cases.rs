use rdf5d::{reader::R5tuFile, writer::{write_file, write_file_with_options, WriterOptions, Quint, Term}, StreamingWriter};

fn mk_temp(name: &str) -> std::path::PathBuf {
    let mut p = std::env::temp_dir(); p.push(name); p
}
...
fn streaming_empty_finalize() {
    ...
    let w = StreamingWriter::new(&path, WriterOptions { zstd: false, with_crc: true });
...
fn postings_monotonicity_and_spo_order() {
    ...
    let o2 = Term::Literal { lex: "v2".into(), dt: None, lang: None };
    let o3 = Term::Literal { lex: "v3".into(), dt: None, lang: None };
    let qs = vec![
        Quint{ id:"A".into(), s:s1.clone(), p:p1.clone(), o:o1.clone(), gname:"g".into() },
        Quint{ id:"A".into(), s:s1.clone(), p:p2.clone(), o:o2.clone(), gname:"g".into() },
        Quint{ id:"B".into(), s:s2.clone(), p:p1.clone(), o:o3.clone(), gname:"g".into() },
    ];
    let path = mk_temp("mono.r5tu");
    write_file_with_options(&path, &qs, WriterOptions{ zstd:false, with_crc:true }).unwrap();
...
fn long_strings_and_lookup() {
    ...
    let q = Quint{ id: long.clone(), s: Term::Iri("http://ex/s".into()), p: Term::Iri("http://ex/p".into()), o: Term::Iri("http://ex/o".into()), gname: long.clone() };
