...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Commands::BuildGraph(args) => {
            let opts = WriterOptions { zstd: args.zstd, with_crc: !args.no_crc };
        ...
        Commands::BuildDataset(args) => {
            let default_g = args.default_graphname.clone().unwrap_or_else(|| "default".to_string());
            let opts = WriterOptions { zstd: args.zstd, with_crc: !args.no_crc };
            let mut w = StreamingWriter::new(&args.output, opts);
            let start = Instant::now();
            for input in args.input {
        ...
        Commands::Stat(args) => {
            ...
            let graphs = if let Some(ref gname) = filter_g { f.enumerate_by_graphname(gname)? } else { f.enumerate_all()? };
