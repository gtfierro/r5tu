...
use crate::header::{parse_footer, parse_toc, section_in_bounds, crc32_ieee, Header, Section, SectionKind, TocEntry};
...
impl R5tuFile {
    pub fn open(path: &Path) -> Result<Self> {
        ...
        for e in &toc {
            if !section_in_bounds(data.len(), e.section) {
                return Err(R5Error::Corrupt(format!("section {:?} out of bounds", e.kind)));
            }
            if e.crc32_u32 != 0 {
                ...
                if got != e.crc32_u32 { return Err(R5Error::Corrupt("section CRC mismatch".into())); }
            }
        }
        // Validate TOC ordering by offset and detect overlaps
        let mut spans: Vec<(u64,u64)> = toc.iter().map(|e| (e.section.off, e.section.len)).collect();
        spans.sort_by_key(|(off,_)| *off);
        for w in spans.windows(2) {
            let (a_off, a_len) = w[0];
            let (b_off, _b_len) = w[1];
            if a_off + a_len > b_off { return Err(R5Error::Corrupt("TOC sections overlap or unsorted".into())); }
        }
        ...
        for (s_id, p_id, o_id) in self.triples_ids(gid)? {
            ...
            let s_nb: NamedOrBlankNode = match s_parts {
                ...
                TermParts::BNode(label) => {
                    ...
                    BlankNode::new(lbl).map_err(|_| R5Error::Invalid("invalid blank node"))?.into()
                }
                TermParts::Literal{..} => return Err(R5Error::Invalid("literal subject")),
            };
            let p_nn = match p_parts {
                TermParts::Iri(p) => NamedNode::new(p).map_err(|_| R5Error::Invalid("invalid predicate IRI"))?,
                _ => return Err(R5Error::Invalid("non-IRI predicate")),
            };
            let o_term: oxigraph::model::Term = match o_parts {
                TermParts::Iri(o) => NamedNode::new(o).map_err(|_| R5Error::Invalid("invalid object IRI"))?.into(),
                TermParts::BNode(label) => {
                    let lbl = label.strip_prefix("_:").unwrap_or(&label).to_string();
                    BlankNode::new(lbl).map_err(|_| R5Error::Invalid("invalid blank node"))?.into()
                }
                TermParts::Literal { lex, dt, lang } => {
                    if let Some(dt) = dt {
                        let nn = NamedNode::new(dt).map_err(|_| R5Error::Invalid("invalid datatype IRI"))?;
                        Literal::new_typed_literal(lex, nn).into()
                    } else if let Some(lang) = lang {
                        Literal::new_language_tagged_literal(lex, lang).map_err(|_| R5Error::Invalid("invalid lang tag"))?.into()
                    } else {
    ...
    pub fn enumerate_all(&self) -> Result<Vec<GraphRef>> {
        ...
        for gid in 0..n_rows { out.push(self.graphref_for_gid(gid)?); }
        Ok(out)
    }
}
...
impl Dict {
    fn parse(data: &[u8], sec: Section) -> Result<Self> {
        ...
        let blob = Section { off: blob_off, len: blob_len };
        let offs = Section { off: offs_off, len: offs_len };
        let idx = if idx_off != 0 { Some(Section { off: idx_off, len: idx_len }) } else { None };
        ...
        Ok(Dict { sec, n, blob, offs, idx })
    }

    fn get<'a>(&self, data: &'a [u8], id: u32) -> Option<&'a str> {
        if id >= self.n { return None; }
        let o_base = self.offs.off as usize;
        let s = u32::from_le_bytes(data[o_base + id as usize * 4 .. o_base + id as usize * 4 + 4].try_into().ok()?) as usize;
        let e = u32::from_le_bytes(data[o_base + (id as usize + 1) * 4 .. o_base + (id as usize + 1) * 4 + 4].try_into().ok()?) as usize;
    ...
    fn find_id(&self, data: &[u8], s: &str) -> Option<u32> {
        if let Some(idx) = self.idx {
            ...
            for (i, b) in s.to_ascii_lowercase().as_bytes().iter().take(16).enumerate() {
                key16[i] = *b;
            }
            let mut lo = 0usize;
            let mut hi = n;
...
impl TermDict {
    fn parse(data: &[u8], sec: Section) -> Result<Self> {
        if !section_in_bounds(data.len(), sec) { return Err(R5Error::Corrupt("term dict OOB".into())); }
        let base = sec.off as usize;
        if base + 1 + 8 * 4 > data.len() { return Err(R5Error::Corrupt("short term dict header".into())); }
        ...
        Ok(TermDict { n_terms, kinds_off, data_off, offs_off })
    }
    ...
    fn term_to_string(&self, data: &[u8], term_id: u64) -> Result<String> {
        ...
        // offs is u64 * (n+1)
        let s = u64::from_le_bytes(data[offs_off + term_id as usize * 8 .. offs_off + term_id as usize * 8 + 8].try_into().unwrap()) as usize;
        let e = u64::from_le_bytes(data[offs_off + (term_id as usize + 1) * 8 .. offs_off + (term_id as usize + 1) * 8 + 8].try_into().unwrap()) as usize;
        let payload = &data[data_off + s .. data_off + e];
        match data[kinds_off + term_id as usize] {
            0 | 1 => std::str::from_utf8(payload).map(String::from).map_err(|_| R5Error::Corrupt("utf8".into())),
            2 => {
                let (lex_len, mut off) = read_uvarint(payload, 0).ok_or_else(|| R5Error::Corrupt("lit lex".into()))?;
                let lex = std::str::from_utf8(&payload[off .. off + lex_len as usize]).map_err(|_| R5Error::Corrupt("utf8".into()))?;
                off += lex_len as usize;
                if off >= payload.len() { return Err(R5Error::Corrupt("lit bounds".into())); }
                let has_dt = payload[off];
                off += 1;
                let dt = if has_dt == 1 {
                    let (l, o2) = read_uvarint(payload, off).ok_or_else(|| R5Error::Corrupt("dt len".into()))?;
                    let s = std::str::from_utf8(&payload[o2 .. o2 + l as usize]).map_err(|_| R5Error::Corrupt("utf8".into()))?;
                    off = o2 + l as usize;
                    Some(s.to_string())
                } else { None };
                if off >= payload.len() { return Err(R5Error::Corrupt("lit bounds2".into())); }
                let has_lang = payload[off];
                off += 1;
                let lang = if has_lang == 1 {
                    let (l, o2) = read_uvarint(payload, off).ok_or_else(|| R5Error::Corrupt("lang len".into()))?;
                    let s = std::str::from_utf8(&payload[o2 .. o2 + l as usize]).map_err(|_| R5Error::Corrupt("utf8".into()))?;
                    Some(s.to_string())
                } else { None };
                Ok(match (dt, lang) {
    ...
    fn term_parts(&self, data: &[u8], term_id: u64) -> Result<TermParts> {
        ...
        Ok(match data[kinds_off + term_id as usize] {
            0 => TermParts::Iri(std::str::from_utf8(payload).map_err(|_| R5Error::Corrupt("utf8".into()))?.to_string()),
            1 => TermParts::BNode(std::str::from_utf8(payload).map_err(|_| R5Error::Corrupt("utf8".into()))?.to_string()),
            2 => {
                let (lex_len, mut off) = read_uvarint(payload, 0).ok_or_else(|| R5Error::Corrupt("lit lex".into()))?;
                let lex = std::str::from_utf8(&payload[off .. off + lex_len as usize]).map_err(|_| R5Error::Corrupt("utf8".into()))?.to_string();
                off += lex_len as usize;
                if off >= payload.len() { return Err(R5Error::Corrupt("lit bounds".into())); }
                let has_dt = payload[off]; off += 1;
                let dt = if has_dt == 1 {
                    let (l, o2) = read_uvarint(payload, off).ok_or_else(|| R5Error::Corrupt("dt len".into()))?; off = o2;
                    let s = std::str::from_utf8(&payload[off .. off + l as usize]).map_err(|_| R5Error::Corrupt("utf8".into()))?.to_string();
                    off += l as usize;
                    Some(s)
                } else { None };
                if off >= payload.len() { return Err(R5Error::Corrupt("lit bounds2".into())); }
                let has_lang = payload[off]; off += 1;
                let lang = if has_lang == 1 {
                    let (l, o2) = read_uvarint(payload, off).ok_or_else(|| R5Error::Corrupt("lang len".into()))?; off = o2;
                    let s = std::str::from_utf8(&payload[off .. off + l as usize]).map_err(|_| R5Error::Corrupt("utf8".into()))?.to_string();
                    Some(s)
                } else { None };
                TermParts::Literal { lex, dt, lang }
...
impl<'a> Iterator for OxTripleIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let s_nb: NamedOrBlankNode = match s_parts {
            ...
            TermParts::BNode(label) => {
                let lbl = label.strip_prefix("_:").unwrap_or(&label).to_string();
                match BlankNode::new(lbl) { Ok(b) => b.into(), Err(_) => return Some(Err(R5Error::Invalid("invalid blank node"))) }
            }
            TermParts::Literal{..} => return Some(Err(R5Error::Invalid("literal subject"))),
        };
        let p_nn = match p_parts {
            TermParts::Iri(p) => match NamedNode::new(p) { Ok(n) => n, Err(_) => return Some(Err(R5Error::Invalid("invalid predicate IRI"))) },
            _ => return Some(Err(R5Error::Invalid("non-IRI predicate"))),
        };
        let o_term: oxigraph::model::Term = match o_parts {
            TermParts::Iri(o) => match NamedNode::new(o) { Ok(n) => n.into(), Err(_) => return Some(Err(R5Error::Invalid("invalid object IRI"))) },
            TermParts::BNode(label) => {
                let lbl = label.strip_prefix("_:").unwrap_or(&label).to_string();
                match BlankNode::new(lbl) { Ok(b) => b.into(), Err(_) => return Some(Err(R5Error::Invalid("invalid blank node"))) }
            }
            TermParts::Literal { lex, dt, lang } => {
                if let Some(dt) = dt {
                    let nn = match NamedNode::new(dt) { Ok(n) => n, Err(_) => return Some(Err(R5Error::Invalid("invalid datatype IRI"))) };
                    Literal::new_typed_literal(lex, nn).into()
                } else if let Some(lang) = lang {
                    match Literal::new_language_tagged_literal(lex, lang) { Ok(l) => l.into(), Err(_) => return Some(Err(R5Error::Invalid("invalid lang tag"))) }
                } else {
                    Literal::new_simple_literal(lex).into()
                }
            }
...
// ---------------- GDIR and GraphRefs ----------------
#[derive(Debug, Clone, Copy)]
struct GDirRow { id_id: u32, gn_id: u32, triples_off: u64, triples_len: u64, n_triples: u64, #[allow(dead_code)] n_s: u32, #[allow(dead_code)] n_p: u32, #[allow(dead_code)] n_o: u32 }
...
impl R5tuFile {
    fn postings_to_graphrefs(&self, sec: Section, key_ordinal: usize) -> Result<Vec<GraphRef>> {
        ...
        for gid in gids { out.push(self.graphref_for_gid(gid)?); }
        Ok(out)
    }
...
impl Iterator for TripleIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.oi >= self.o_vals.len() { return None; }
        while self.pi + 1 < self.p_heads.len() && self.p_heads[self.pi + 1] <= self.oi as u64 { self.pi += 1; }
        while self.si + 1 < self.s_heads.len() && self.s_heads[self.si + 1] <= self.pi as u64 { self.si += 1; }
...
impl R5tuFile {
    ...
    fn decode_raw_payload(&self, raw: &[u8]) -> Result<TripleIter> {
        let mut off = 0usize;
        let (n_s, o1) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("nS".into()))?; off = o1;
        let (n_p, o2) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("nP".into()))?; off = o2;
        let (n_t, o3) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("nT".into()))?; off = o3;
        let n_s = n_s as usize; let n_p = n_p as usize; let n_t = n_t as usize;
        // S_vals (delta-coded ascending)
        let mut s_vals = Vec::with_capacity(n_s);
        if n_s > 0 {
            let (first, o) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("S first".into()))?; off = o; s_vals.push(first);
            for _ in 1..n_s {
                let (d, o2) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("S delta".into()))?; off = o2;
                let prev = *s_vals.last().unwrap();
                s_vals.push(prev.checked_add(d).ok_or_else(|| R5Error::Corrupt("S overflow".into()))?);
            }
        }
        // S_heads (prefix sums into P)
        let mut s_heads = Vec::with_capacity(n_s + 1);
        for _ in 0..(n_s + 1) { let (v, o) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("S_heads".into()))?; off = o; s_heads.push(v); }
        if *s_heads.last().unwrap_or(&0) as usize != n_p { return Err(R5Error::Corrupt("S_heads last != nP".into())); }

        // P_vals (delta-coded per S-run)
        let mut p_vals = vec![0u64; n_p];
        for s in 0..n_s {
            let start = s_heads[s] as usize; let end = s_heads[s + 1] as usize;
            if start > end || end > n_p { return Err(R5Error::Corrupt("P run OOB".into())); }
            if start == end { continue; }
            // first absolute in run
            let (first, o) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("P first".into()))?; off = o; p_vals[start] = first;
            let mut cur = first;
            for i in (start + 1)..end {
                let (d, o2) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("P delta".into()))?; off = o2;
                cur = cur.checked_add(d).ok_or_else(|| R5Error::Corrupt("P overflow".into()))?;
                p_vals[i] = cur;
            }
        }
        // P_heads (prefix sums into O)
        let mut p_heads = Vec::with_capacity(n_p + 1);
        for _ in 0..(n_p + 1) { let (v, o) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("P_heads".into()))?; off = o; p_heads.push(v); }
        if *p_heads.last().unwrap_or(&0) as usize != n_t { return Err(R5Error::Corrupt("P_heads last != nT".into())); }

        // O_vals (delta-coded per (S,P)-run)
        let mut o_vals = vec![0u64; n_t];
        for p in 0..n_p {
            let start = p_heads[p] as usize; let end = p_heads[p + 1] as usize;
            if start > end || end > n_t { return Err(R5Error::Corrupt("O run OOB".into())); }
            if start == end { continue; }
            let (first, o) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("O first".into()))?; off = o; o_vals[start] = first;
            let mut cur = first;
            for i in (start + 1)..end {
                let (d, o2) = read_uvarint(raw, off).ok_or_else(|| R5Error::Corrupt("O delta".into()))?; off = o2;
                cur = cur.checked_add(d).ok_or_else(|| R5Error::Corrupt("O overflow".into()))?;
                o_vals[i] = cur;
            }
        }

        Ok(TripleIter { s_vals, s_heads, p_vals, p_heads, o_vals, si: 0, pi: 0, oi: 0 })
    }
}
...
mod tests {
    ...
    fn uvarint_roundtrip_and_bounds() {
        let mut buf = Vec::new();
        for &n in &[0, 1, 127, 128, 255, 16384, u32::MAX as u64, u64::from(u32::MAX) + 12345] {
            buf.clear();
    ...
    fn dict_parse_and_lookup() {
        ...
        // offs [0,1,2]
        for n in [0u32, 1, 2] { file.extend_from_slice(&n.to_le_bytes()); }
        // fill header
        let n = 2u32; file[sec_off..sec_off+4].copy_from_slice(&n.to_le_bytes());
        ...
        let dict = Dict::parse(&file, Section { off: sec_off as u64, len: (file.len() - sec_off) as u64 }).unwrap();
    ...
    fn term_dict_decode() {
        ...
        push_uvarint(2, &mut lit1); lit1.extend_from_slice(b"42"); // lex
        lit1.push(1); // has_dt
        push_uvarint(11, &mut lit1); lit1.extend_from_slice(b"http://ex/i");
        lit1.push(0); // has_lang
        file.extend_from_slice(&lit1);
        // Literal with lang: "en"@en
        let mut lit2 = Vec::new();
        push_uvarint(2, &mut lit2); lit2.extend_from_slice(b"en");
        lit2.push(0); // no dt
        ...
        file.extend_from_slice(&cur.to_le_bytes()); cur += sizes[0];
        file.extend_from_slice(&cur.to_le_bytes()); cur += sizes[1];
        file.extend_from_slice(&cur.to_le_bytes()); cur += sizes[2];
        file.extend_from_slice(&cur.to_le_bytes());
        // fill header
        file[sec_off] = 0; // width
        let n_terms = (3u64).to_le_bytes(); file[sec_off+1..sec_off+9].copy_from_slice(&n_terms);
        ...
        let td = TermDict::parse(&file, Section { off: sec_off as u64, len: (file.len() - sec_off) as u64 }).unwrap();
        assert_eq!(td.term_to_string(&file, 0).unwrap(), "http://ex/s");
        assert_eq!(td.term_to_string(&file, 1).unwrap(), "\"42\"^^<http://ex/i>");
    ...
    fn end_to_end_minimal_file() {
        ...
        // ID_DICT with ["A"]
        let id_sec_off = f.len(); f.resize(f.len()+52, 0);
        let id_blob_off = f.len(); f.extend_from_slice(b"A");
        let id_offs_off = f.len(); for n in [0u32,1] { f.extend_from_slice(&n.to_le_bytes()); }
        // fill header
        ...
        // GNAME_DICT with ["g"]
        let gn_sec_off = f.len(); f.resize(f.len()+52, 0);
        let gn_blob_off = f.len(); f.extend_from_slice(b"g");
        let gn_offs_off = f.len(); for n in [0u32,1] { f.extend_from_slice(&n.to_le_bytes()); }
        ...
        let td_sec_off = f.len(); f.resize(f.len()+33, 0);
        let kinds_off = f.len(); // empty kinds
        let data_off = f.len(); // empty data
        let offs_off = f.len(); f.extend_from_slice(&0u64.to_le_bytes()); // single 0
        f[td_sec_off] = 0;
        ...
        let mut raw = Vec::new();
        // nS=1, nP=2, nT=2
        push_uvarint(1, &mut raw); push_uvarint(2, &mut raw); push_uvarint(2, &mut raw);
        // S_vals: [1]
        push_uvarint(1, &mut raw);
        // S_heads: [0,2]
        push_uvarint(0, &mut raw); push_uvarint(2, &mut raw);
        // P_vals for S run: [2,4]
        push_uvarint(2, &mut raw); // first absolute
        push_uvarint(2, &mut raw); // delta = 2 (4-2)
        // P_heads: [0,1,2]
        push_uvarint(0, &mut raw); push_uvarint(1, &mut raw); push_uvarint(2, &mut raw);
        // O_vals per P run: [3] then [5]
        push_uvarint(3, &mut raw); push_uvarint(5, &mut raw);
        // block header
        f.push(0u8); // enc = RAW
        let raw_len = raw.len() as u32; f.extend_from_slice(&raw_len.to_le_bytes());
        ...
        let ididx_sec_off = f.len(); let ididx_hdr_off = ididx_sec_off; f.resize(f.len()+24, 0);
        let ididx_offs_off = f.len(); for n in [0u64, 2u64] { f.extend_from_slice(&n.to_le_bytes()); } // blob len 2 bytes
        let ididx_blob_off = f.len(); let mut tmp = Vec::new(); push_uvarint(1, &mut tmp); push_uvarint(0, &mut tmp); f.extend_from_slice(&tmp);
        // header
        f[ididx_hdr_off..ididx_hdr_off+8].copy_from_slice(&1u64.to_le_bytes());
        f[ididx_hdr_off+8..ididx_hdr_off+16].copy_from_slice(&(ididx_offs_off as u64).to_le_bytes());
        f[ididx_hdr_off+16..ididx_hdr_off+24].copy_from_slice(&(ididx_blob_off as u64).to_le_bytes());
        add_sec(SectionKind::IdxId2Gid, ididx_sec_off, f.len()-ididx_sec_off);

        // IDX_GNAME2GID same
        let gnidx_sec_off = f.len(); let gnidx_hdr_off = gnidx_sec_off; f.resize(f.len()+24, 0);
        let gnidx_offs_off = f.len(); for n in [0u64, 2u64] { f.extend_from_slice(&n.to_le_bytes()); }
        let gnidx_blob_off = f.len(); let mut tmp2 = Vec::new(); push_uvarint(1, &mut tmp2); push_uvarint(0, &mut tmp2); f.extend_from_slice(&tmp2);
        f[gnidx_hdr_off..gnidx_hdr_off+8].copy_from_slice(&1u64.to_le_bytes());
        f[gnidx_hdr_off+8..gnidx_hdr_off+16].copy_from_slice(&(gnidx_offs_off as u64).to_le_bytes());
        f[gnidx_hdr_off+16..gnidx_hdr_off+24].copy_from_slice(&(gnidx_blob_off as u64).to_le_bytes());
        add_sec(SectionKind::IdxGName2Gid, gnidx_sec_off, f.len()-gnidx_sec_off);
        ...
        add_sec(SectionKind::IdxPair2Gid, pairidx_sec_off, f.len()-pairidx_sec_off);

        // TOC
        let toc_off = f.len();
        for (kind, off, len) in &toc_entries {
            let mut ent = [0u8; 32];
            let kind_u16 = *kind as u16; ent[0..2].copy_from_slice(&kind_u16.to_le_bytes());
        ...
        // Write and open
        let mut path = std::env::temp_dir(); path.push("mini.r5tu");
        let mut file = fs::File::create(&path).unwrap(); file.write_all(&f).unwrap();
    ...
    fn rejects_overlapping_toc_sections() {
        ...
        // add a blob region
        let s1_off = f.len(); f.extend_from_slice(&[0u8; 100]);
        let s2_off = s1_off + 50; // intentional overlap
        f.extend_from_slice(&[0u8; 20]);
        // TOC
        let toc_off = f.len();
        let mut ent1 = [0u8; 32]; ent1[0..2].copy_from_slice(&(SectionKind::IdDict as u16).to_le_bytes()); ent1[4..12].copy_from_slice(&(s1_off as u64).to_le_bytes()); ent1[12..20].copy_from_slice(&(100u64).to_le_bytes());
        let mut ent2 = [0u8; 32]; ent2[0..2].copy_from_slice(&(SectionKind::GNameDict as u16).to_le_bytes()); ent2[4..12].copy_from_slice(&(s2_off as u64).to_le_bytes()); ent2[12..20].copy_from_slice(&(20u64).to_le_bytes());
        f.extend_from_slice(&ent1); f.extend_from_slice(&ent2);
        // Header
        f[0..4].copy_from_slice(b"R5TU");
        f[4..6].copy_from_slice(&1u16.to_le_bytes());
        f[6..8].copy_from_slice(&0u16.to_le_bytes());
        ...
        let mut path = std::env::temp_dir(); path.push("overlap.r5tu");
        let mut file = fs::File::create(&path).unwrap(); file.write_all(&f).unwrap();
        let err = R5tuFile::open(&path).unwrap_err();
        match err { R5Error::Corrupt(m) => assert!(m.contains("overlap")), _ => panic!("expected Corrupt overlap") }
        let _ = fs::remove_file(&path);
    }
}
