...
use crate::reader::{Result, R5Error};

#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum Term {
    Iri(String),
    BNode(String),
    Literal { lex: String, dt: Option<String>, lang: Option<String> },
}
...
pub struct WriterOptions { pub zstd: bool, pub with_crc: bool }

pub fn write_file<P: AsRef<Path>>(path: P, quads: &[Quint]) -> Result<()> {
    write_file_with_options(path, quads, WriterOptions{ zstd: false, with_crc: true })
}

pub fn write_file_with_options<P: AsRef<Path>>(path: P, quads: &[Quint], opts: WriterOptions) -> Result<()> {
    ...
    let mut intern_id = |s: &str| -> u32 {
        if let Some(&v) = id_map.get(s) { return v; }
        let v = id_vec.len() as u32; id_vec.push(s.to_string()); id_map.insert(s.to_string(), v); v
    };
    let mut intern_gn = |s: &str| -> u32 {
        if let Some(&v) = gn_map.get(s) { return v; }
        let v = gn_vec.len() as u32; gn_vec.push(s.to_string()); gn_map.insert(s.to_string(), v); v
    };
    let mut intern_term = |t: &Term| -> u64 {
        if let Some(&v) = term_map.get(t) { return v; }
        let v = term_vec.len() as u64; term_vec.push(t.clone()); term_map.insert(t.clone(), v); v
    };
    ...
    for (id_id, gn_id, s, p, o) in triples { groups.entry((id_id, gn_id)).or_default().push((s, p, o)); }
    for v in groups.values_mut() { v.sort_unstable(); }
    ...
    // ID_DICT
    let id_sec = write_str_dict(&mut file, &id_vec)?; toc.push(TocEntry{ kind: SectionKind::IdDict, section: id_sec, crc32_u32: 0});
    // GNAME_DICT
    let gn_sec = write_str_dict(&mut file, &gn_vec)?; toc.push(TocEntry{ kind: SectionKind::GNameDict, section: gn_sec, crc32_u32: 0});
    // TERM_DICT
    let term_sec = write_term_dict(&mut file, &term_vec)?; toc.push(TocEntry{ kind: SectionKind::TermDict, section: term_sec, crc32_u32: 0});
    ...
    for ((id_id, gn_id), spo) in &groups {
        ...
        if opts.zstd {
            #[cfg(feature = "zstd")]
            {
                ...
                let compressed = zstd::encode_all(&raw[..], 0).map_err(|_| R5Error::Corrupt("zstd encode".into()))?;
            ...
            // RAW
            file.push(0u8);
            let raw_len = raw.len() as u32; file.extend_from_slice(&raw_len.to_le_bytes());
            file.extend_from_slice(&raw);
        }
        let sec = Section{ off: start as u64, len: (file.len()-start) as u64 };
        // counts
        let (n_s, n_p, n_t) = raw_counts(&raw)?;
        gid_rows.push((*id_id, *gn_id, sec, n_t as u64, n_s as u32, n_p as u32, n_t as u32));
    }
    let tb_sec = Section{ off: tb_off as u64, len: (file.len()-tb_off) as u64 };
    toc.push(TocEntry{ kind: SectionKind::TripleBlocks, section: tb_sec, crc32_u32: 0});
    ...
    let gdir_sec = Section{ off: gdir_off as u64, len: (file.len()-gdir_off) as u64 };
    toc.push(TocEntry{ kind: SectionKind::GDir, section: gdir_sec, crc32_u32: 0});
    ...
    toc.push(TocEntry{ kind: SectionKind::IdxId2Gid, section: ididx_sec, crc32_u32: 0});
    // IDX_GNAME2GID
    let gnidx_sec = write_postings_index(&mut file, &gn2gids)?;
    toc.push(TocEntry{ kind: SectionKind::IdxGName2Gid, section: gnidx_sec, crc32_u32: 0});
    // IDX_PAIR2GID
    let pairidx_sec = write_pair_index(&mut file, &pair_entries)?;
    toc.push(TocEntry{ kind: SectionKind::IdxPair2Gid, section: pairidx_sec, crc32_u32: 0});

    // TOC
    let toc_off = file.len();
    for e in &toc {
        let mut ent = [0u8; 32];
        let kind = e.kind as u16; ent[0..2].copy_from_slice(&kind.to_le_bytes());
        // reserved_u16 zero
    ...
    let mut flags: u16 = 0; if opts.zstd { flags |= 1<<1; }
...
impl StreamingWriter {
    ...
    fn intern_id(&mut self, s: &str) -> u32 {
        if let Some(&v) = self.id_map.get(s) { return v; }
        let v = self.id_vec.len() as u32; self.id_vec.push(s.to_string()); self.id_map.insert(s.to_string(), v); v
    }
    fn intern_gn(&mut self, s: &str) -> u32 {
        if let Some(&v) = self.gn_map.get(s) { return v; }
        let v = self.gn_vec.len() as u32; self.gn_vec.push(s.to_string()); self.gn_map.insert(s.to_string(), v); v
    }
    fn intern_term(&mut self, t: &Term) -> u64 {
        if let Some(&v) = self.term_map.get(t) { return v; }
        let v = self.term_vec.len() as u64; self.term_vec.push(t.clone()); self.term_map.insert(t.clone(), v); v
    }
    ...
    pub fn add(&mut self, q: Quint) -> Result<()> {
        ...
        self.groups.entry((id_id, gn_id)).or_default().push((s,p,o));
        Ok(())
    }

    pub fn finalize(mut self) -> Result<()> {
        // Ensure per-group SPO sort
        for v in self.groups.values_mut() { v.sort_unstable(); }

        // Build buffers using the same logic as write_file_with_options
        let mut file = vec![0u8; 32];
        let mut toc: Vec<TocEntry> = Vec::new();

        let id_sec = write_str_dict(&mut file, &self.id_vec)?; toc.push(TocEntry{ kind: SectionKind::IdDict, section: id_sec, crc32_u32: 0});
        let gn_sec = write_str_dict(&mut file, &self.gn_vec)?; toc.push(TocEntry{ kind: SectionKind::GNameDict, section: gn_sec, crc32_u32: 0});
        let term_sec = write_term_dict(&mut file, &self.term_vec)?; toc.push(TocEntry{ kind: SectionKind::TermDict, section: term_sec, crc32_u32: 0});
        ...
        for e in &toc {
            ...
            if self.opts.with_crc {
                let start = e.section.off as usize; let end = start + e.section.len as usize;
                let crc = crc32_ieee(&file[start..end]); ent[20..24].copy_from_slice(&crc.to_le_bytes());
            }
            file.extend_from_slice(&ent);
        }
        ...
        let mut flags: u16 = 0; if self.opts.zstd { flags |= 1<<1; }
        file[6..8].copy_from_slice(&flags.to_le_bytes());
...
fn term_from_ox_term_ref(t: &oxigraph::model::TermRef<'_>) -> Term {
    ...
    match t {
        ...
        TR::Literal(l) => {
            ...
            if let Some(lang) = l.language() { Term::Literal{ lex, dt: None, lang: Some(lang.to_string()) } }
            else { Term::Literal{ lex, dt: Some(l.datatype().as_str().to_string()), lang: None } }
        }
...
#[cfg(feature = "oxigraph")]
impl StreamingWriter {
    pub fn add_oxigraph_graph(&mut self, graph: &oxigraph::model::Graph, id: &str, gname: &str) -> Result<()> {
        ...
        for t in graph.iter() {
            ...
            self.add(Quint { id: id.to_string(), s, p, o, gname: gname.to_string() })?;
        }
        Ok(())
    }
}
...
#[cfg(feature = "oxigraph")]
pub fn write_graph_from_oxigraph<P: AsRef<Path>>(path: P, graph: &oxigraph::model::Graph, id: &str, gname: &str, opts: WriterOptions) -> Result<()> {
...
#[cfg(feature = "oxigraph")]
pub fn write_graph_from_oxigraph_auto<P: AsRef<Path>>(path: P, graph: &oxigraph::model::Graph, opts: WriterOptions) -> Result<()> {
...
pub fn detect_graphname_from_store(store: &oxigraph::store::Store) -> Option<String> {
    use oxigraph::model::{NamedNode, TermRef, GraphNameRef};
    let rdf_type = NamedNode::new("http://www.w3.org/1999/02/22-rdf-syntax-ns#type").ok()?;
    let owl_ontology = NamedNode::new("http://www.w3.org/2002/07/owl#Ontology").ok()?;
    let mut it = store.quads_for_pattern(None, Some(rdf_type.as_ref()), Some(TermRef::NamedNode(owl_ontology.as_ref())), Some(GraphNameRef::DefaultGraph));
...
fn write_str_dict(buf: &mut Vec<u8>, strings: &[String]) -> Result<Section> {
    ...
    for s in strings { buf.extend_from_slice(s.as_bytes()); }
    let blob_len = buf.len() - blob_off;
    let offs_off = buf.len();
    // offs len = (n+1) * 4
    let mut cur = 0u32;
    for s in strings { buf.extend_from_slice(&cur.to_le_bytes()); cur = cur.checked_add(s.len() as u32).ok_or_else(|| R5Error::Corrupt("blob size".into()))?; }
    ...
    for (i, s) in strings.iter().enumerate() {
        let mut key = [0u8;16];
        for (j, b) in s.to_ascii_lowercase().as_bytes().iter().take(16).enumerate() { key[j] = *b; }
    ...
    } else { idx_off = 0; idx_len = 0; }
    // fill header
    let n = strings.len() as u32; buf[off..off+4].copy_from_slice(&n.to_le_bytes());
    ...
    Ok(Section{ off: off as u64, len: (buf.len()-off) as u64 })
}
...
fn write_term_dict(buf: &mut Vec<u8>, terms: &[Term]) -> Result<Section> {
    ...
    for t in terms {
        buf.push(match t { Term::Iri(_) => 0, Term::BNode(_) => 1, Term::Literal{..} => 2 });
    }
    // data blob
    let data_off = buf.len();
    let mut offs: Vec<u64> = Vec::with_capacity(terms.len()+1);
    let mut cur: u64 = 0; offs.push(cur);
    for t in terms {
        match t {
            Term::Iri(s) | Term::BNode(s) => { buf.extend_from_slice(s.as_bytes()); cur += s.len() as u64; }
            Term::Literal { lex, dt, lang } => {
                push_uvarint(lex.len() as u64, buf); buf.extend_from_slice(lex.as_bytes());
                match dt {
                    Some(d) => { buf.push(1); push_uvarint(d.len() as u64, buf); buf.extend_from_slice(d.as_bytes()); }
                    None => buf.push(0),
                }
                match lang {
                    Some(l) => { buf.push(1); push_uvarint(l.len() as u64, buf); buf.extend_from_slice(l.as_bytes()); }
                    None => buf.push(0),
                }
...
fn build_raw_spo(spo: &[(u64,u64,u64)]) -> Result<Vec<u8>> {
    ...
    if !s_vals.is_empty() {
        ...
        for (idx, v) in s_vals.iter().enumerate() {
            if idx==0 { push_uvarint(*v, &mut out); prev=*v; }
            else { push_uvarint(v.checked_sub(prev).ok_or_else(|| R5Error::Corrupt("s delta underflow".into()))?, &mut out); prev=*v; }
        }
    }
    // S_heads
    for v in &s_heads { push_uvarint(*v, &mut out); }
    ...
    for s_idx in 0..s_vals.len() {
        ...
        for (i, idx) in (start..end).enumerate() {
            let v = p_vals[idx];
            if i==0 { push_uvarint(v, &mut out); prev=v; }
            else { push_uvarint(v.checked_sub(prev).ok_or_else(|| R5Error::Corrupt("p delta underflow".into()))?, &mut out); prev=v; }
        }
    }
    // P_heads
    for v in &p_heads { push_uvarint(*v, &mut out); }
    ...
    for p_idx in 0..p_vals.len() {
        ...
        for (i, idx) in (start..end).enumerate() {
            let v = o_vals[idx];
            if i==0 { push_uvarint(v, &mut out); prev=v; }
            else { push_uvarint(v.checked_sub(prev).ok_or_else(|| R5Error::Corrupt("o delta underflow".into()))?, &mut out); prev=v; }
        }
    }
    Ok(out)
}
...
fn write_postings_index(buf: &mut Vec<u8>, lists: &[Vec<u64>]) -> Result<Section> {
    ...
    let offs_off = buf.len();
    let mut cur = 0u64; buf.extend_from_slice(&cur.to_le_bytes());
    let mut blob = Vec::new();
    for list in lists {
        // encode list
        if list.is_empty() { push_uvarint(0, &mut blob); }
        else {
            push_uvarint(list.len() as u64, &mut blob);
            push_uvarint(list[0], &mut blob);
            for w in list.windows(2) { push_uvarint(w[1]-w[0], &mut blob); }
        }
